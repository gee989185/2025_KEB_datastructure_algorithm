...
def bfs(root):
    if root is None:
        return
    queue = deque([root])  # BFS > queue쓴다고 함. 외워보기
    while queue:
        root = queue.popleft()  # 먼저 들어온 노드 꺼내기(dequeue)
        print(f"{root.data} ", end='')  # 노드 값 출력(enqueue)
        if root.left:
            queue.append(root.left)  # 왼쪽 자식 추가
        if root.right:
            queue.append(root.right)  # 오른쪽 자식 추가
    print()
